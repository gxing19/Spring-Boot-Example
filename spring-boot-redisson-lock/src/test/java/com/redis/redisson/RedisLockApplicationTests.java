...
import com.redis.redisson.config.RedisLockUtil;
...
public class RedisLockApplicationTests {
    ...
    public void testToLock() throws InterruptedException {
        ...
        try {
            boolean lockResult = redisLockUtil.tryLock(lockKey, lockValue, 2);
            System.out.println("lock result: " + lockResult);
            Thread.sleep(3 * 1000);

        } finally {
            boolean releaseResult = redisLockUtil.unlock(lockKey, lockValue);
    ...
    public void releaseLock() throws InterruptedException {
        ...
        boolean lockResult = redisLockUtil.tryLock("goods_id_10001", String.valueOf(threadId), 10);
